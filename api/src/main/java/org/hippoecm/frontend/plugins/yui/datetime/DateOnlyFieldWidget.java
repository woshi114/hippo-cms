...
public class DateOnlyFieldWidget extends Panel {

    private static final long serialVersionUID = 1L;

    public DateOnlyFieldWidget(String id, IModel<Date> model, IPluginContext context, IPluginConfig config) {
        super(id, model);
        ...
        YuiDateField dateField = new YuiDateField("widget", model, settings);
        boolean todayLinkVisible = config.getAsBoolean("show.today.button", true);
        dateField.setTodayLinkVisible(todayLinkVisible);
        add(dateField);
    }
}
