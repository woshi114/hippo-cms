...
public class RenameDocumentDialog extends AbstractWorkflowDialog<RenameDocumentArguments> {
    ...
    public RenameDocumentDialog(RenameDocumentArguments renameDocumentModel, IModel title, IWorkflowInvoker invoker, final StringCodec nodeNameCodec) {
        super(new Model(renameDocumentModel), invoker);
        this.title = title;
        ...
        AjaxLink<Boolean> uriAction = new AjaxLink<Boolean>("uriAction") {
            ...
            public void onClick(AjaxRequestTarget target) {
                ...
                if (!uriModified) {
                    uriModel.setObject(Strings.isEmpty(nameModel.getObject()) ? "" : nodeNameCodec.encode(
                            nameModel.getObject()));
