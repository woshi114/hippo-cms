...
public class ExternalLinkDialog extends AbstractXinhaDialog<ExternalXinhaLink> {
    ...
    public ExternalLinkDialog(IPluginContext context, IPluginConfig config, IModel<ExternalXinhaLink> model) {
        ...
        List<String> protocols = ExternalXinhaLink.PROTOCOLS;
        if (config.containsKey(AbstractXinhaPlugin.EXTERNAL_PROTOCOLS_OVERWRITE_DEFAULT)) {
            protocols = Arrays.asList(config.getStringArray(AbstractXinhaPlugin.EXTERNAL_PROTOCOLS_OVERWRITE_DEFAULT));
        }

        final DropDownChoice<String> protocolsChoice = new DropDownChoice<String>("protocols",
                new PropertyModel<String>(model, "protocol"), protocols);
